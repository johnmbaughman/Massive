...
namespace Massive.Tests
{
    ...
    {
        private static MockDbConnection _testConnection;

        public WriteTests()
        {
            _testConnection = new MockDbConnection();
        }

        	[Fact]
        	public void Insert_SingleRow()
        	{
                var categories = new Category(_testConnection, true);
                _testConnection.Mocks.When(cmd => cmd.CommandText.Contains("INSERT")).ReturnsScalar(1);
